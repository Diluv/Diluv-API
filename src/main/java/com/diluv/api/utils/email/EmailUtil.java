...
public class EmailUtil {
    public static MessageResponse sendVerificationEmail (String email, String verificationCode) {
        ...
        try {
            ApiClient client = Postmark.getApiClient(Constants.POSTMARK_API_TOKEN);
            String url = String.format("%s/validate?code=%s&email=%s", Constants.WEBSITE_URL, verificationCode, email);
            if (Constants.EMAIL_VERIFICATION == null) {
                DiluvAPI.LOGGER.error("Failed to load email_verification.html.");
                return null;
            }
            Message message = new Message(Constants.NOREPLY_EMAIL, email, "Diluv Email Verification", Constants.EMAIL_VERIFICATION.replace("{{action_url}}", url));
